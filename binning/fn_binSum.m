function outmat = fn_binSum(mat, bin,dim)
...
% 
if dim==1; outmat = fn_cell2mat(cellfun(@(x)(sum(mat(x,:),1)),binCell,'UniformOutput',false) ,1);
elseif dim==2; outmat = fn_cell2mat(cellfun(@(x)(sum(mat(:,x),2)),binCell,'UniformOutput',false) ,2);
end
